...
graph = os.path.join(BASE_DIR, "report3_plot.png")
folder = "./" # find all files
files = glob.glob(os.path.join(folder, "sdc35_report3_*.csv"))
...
## --- Generate report ---
def generate_report3_pdf(plot_file, logo_left, logo_right, table_funcs=[]):
    #--create plot
    timestamps = []
    pv_values = []
    ...
    content = []
    content.append(Paragraph("Report3: Process Control", styles["Title"]))
    content.append(Paragraph(f"Date: {datetime.now().strftime('%Y-%m-%d')} | Report3 | Measure the temperature of the PV process for 1 minute", styles["BodyText"]))
...
build_result["build_report3"] = round(end - start, 6)

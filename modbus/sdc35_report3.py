...
now = datetime.now()
filename3 = f"sdc35_report3_{now.strftime('%Y-%m-%d_%H-%M-%S')}.csv"
filename4 = "time_verification.csv"
ser = serial.Serial(PORT, BAUDRATE, timeout=2)
time.sleep(2)  

print("Starting 1-minute logging, sending report3 every 5 seconds")
    ...
    try:
        while time.time() - start_time < duration:
            ...
            while True:
                ...
                if line.startswith("report3 done") or line.startswith("Connected"):
                    break
        ...
        ser.close()
        print(f"Serial port closed. Data saved to {filename3}")

        end = time.perf_counter()
        readcsv_result["readCSV_report3"] = round(end - start, 6)
